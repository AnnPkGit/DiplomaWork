...
public class GetToastWithContentByIdQueryHandler : IRequestHandler<GetToastWithContentByIdQuery, BaseToastWithContentDto>
{
    ...
    public async Task<BaseToastWithContentDto> Handle(GetToastWithContentByIdQuery request, CancellationToken cancellationToken)
    {
        ...
        var toastWithContentQuery = _context.BaseToastsWithContent
            .IgnoreAutoIncludes()
        ...
        {
            case nameof(Toast): 
                var toast = await toastWithContentQuery
                    .OfType<Toast>()
                    .AsSingleQuery()
                    .SingleOrDefaultAsync(t => t.Id == toastWithContentId, cancellationToken);
                return _mapper.Map<ToastDto>(toast);
            case nameof(Reply):
                var reply = await toastWithContentQuery
                    .OfType<Reply>()
                    ...
                    .AsSingleQuery()
                    .SingleOrDefaultAsync(r => r.Id == toastWithContentId, cancellationToken);
                if (reply != null)
                {
            ...
            case nameof(Quote):
                var quote = await toastWithContentQuery
                    .OfType<Quote>()
                    ...
                    .AsSingleQuery()
                    .SingleOrDefaultAsync(q => q.Id == toastWithContentId, cancellationToken);
